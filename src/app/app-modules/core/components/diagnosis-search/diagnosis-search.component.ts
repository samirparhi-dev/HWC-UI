...
export class DiagnosisSearchComponent implements OnInit {
  ...
  fetchSnomedListBasedOnDiagnosisType: string;
  ...
  checkSelectedDiagnosis(item) {
    ...
    if (addedDiagnosis.length > 1) {
      let temp = addedDiagnosis.filter(
        (diagnosis) => diagnosis.conceptID == item.conceptID
      );
      if (temp.length > 0) {
        return true;
      } else {
  ...
  checkCurrentSelection(item) {
    if (this.selectedDiagnosisList.length > 0) {
      let currentSelection = this.selectedDiagnosisList.filter(
        (diagnosis) => diagnosis.conceptID == item.conceptID
      );
  ...
  searchClinicalFindings(term: string, pageNo, diagnosisType): void {
    if (term.length > 2) {
      this.showProgressBar = true;
      this.masterdataService
