...
export class MasterdataService {
  ...
  getVisitDetailMasterData() {
    return this.http.get(this.visitDetailMasterDataUrl).subscribe((res) => {
  ...
  getDoctorMasterData(visitID: string, providerServiceID) {
    let facilityID = 0;
    if (
      JSON.parse(localStorage.getItem("serviceLineDetails")).facilityID !=
        undefined &&
      JSON.parse(localStorage.getItem("serviceLineDetails")).facilityID != null
    ) {
    ...
    if (
      JSON.parse(localStorage.getItem("serviceLineDetails")).vanID !=
        undefined &&
      JSON.parse(localStorage.getItem("serviceLineDetails")).vanID != null
    ) {
  ...
  getJSON(_jsonURL) {
    return this.http.get(_jsonURL).map((response: any) => response.json());
  }

  searchDiagnosisBasedOnPageNo(searchTerm, pageNo, diagnosisType?: string) {
    const body = {
      term: searchTerm,
      pageNo: pageNo,
      type: diagnosisType,
  ...
  getVaccineList(currentImmunizationServiceID) {
    let visitCategoryID = localStorage.getItem("visitCategoryId");
    return this.http
      .get(environment.vaccineListUrl + currentImmunizationServiceID + "/" + visitCategoryID )
      .map((res) => res.json());
  }
}
