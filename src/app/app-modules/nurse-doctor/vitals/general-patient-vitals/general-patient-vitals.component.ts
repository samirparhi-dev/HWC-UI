...
export class GeneralPatientVitalsComponent implements OnInit {
  ...
  hideVitalsFormForNcdScreening: boolean = true;
  ...
  hideVitalsForm() {
    if(this.visitCategory == "NCD screening"){
      this.disablingVitalsSectionSubscription = this.ncdScreeningService.enablingScreeningDiseases$.subscribe(
        (response) => {
          if(response === false) {
  ...
  checkNurseRequirements(medicalForm) {
    ...
    if (this.visitCategory == "NCD screening") {
      if (vitalsForm.controls["height_cm"].errors) {
        required.push(this.currentLanguageSet.vitalsDetails.AnthropometryDataANC_OPD_NCD_PNC.height);
      }
      if (vitalsForm.controls["weight_Kg"].errors) {
        required.push(this.currentLanguageSet.vitalsDetails.AnthropometryDataANC_OPD_NCD_PNC.weight);
      }
    ...
    this.generalVitalsDataSubscription = this.doctorService
      ...
      .subscribe((vitalsData) => {
        if (vitalsData) {
          ...
          if(this.visitCategory == "ANC") {
          this.hrpService.setHeightFromVitals(this.patientVitalsForm.controls['height_cm'].value);
          this.hrpService.setHemoglobinValue(this.patientVitalsForm.controls['hemoglobin'].value);
        }
          // if(this.attendant =="nurse")
          // this.patientVitalsForm.patchValue({
          //   rbsTestResult: null,
          ...
          if (
            ...
            this.patientVitalsForm.controls["hipCircumference_cm"].value !=
              null &&
            (this.visitCategory == "General OPD" || this.visitCategory == "FP & Contraceptive Services")
          ) {
            this.checkHip(
              this.patientVitalsForm.controls["hipCircumference_cm"].value
            );
  ...
  checkHeight(height_cm) {
    ...
    console.log('HRP- height from vitals:', this.patientVitalsForm.controls['height_cm'].value)
    this.hrpService.setHeightFromVitals(this.patientVitalsForm.controls['height_cm'].value);
