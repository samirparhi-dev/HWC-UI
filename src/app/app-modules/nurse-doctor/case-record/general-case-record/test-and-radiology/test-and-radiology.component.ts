...
export class TestAndRadiologyComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if(this.visitCategory.toLowerCase() == "neonatal and infant health care services" || this.visitCategory.toLowerCase() ==  "childhood & adolescent healthcare services") {
 ...
 this.testInVitalsService.vitalRBSTestResultInUpdate$.subscribe(vitalsresp => {
  ...
  getTestResults(visitCategory) {
    this.testResultsSubscription = this.doctorService.populateCaserecordResponse$
      .subscribe(res => {
  ...
  getMMUTestResults(beneficiaryRegID, visitID, visitCategory) {
    ...
    let respObj;
    //Calling TM Reports
    this.doctorService.getMMUCaseRecordAndReferDetails(beneficiaryRegID, visitID, visitCategory, localStorage.getItem("visitCode"))
      .subscribe(res => {
        console.log('response archive', res);
        if (res && res.statusCode == 200 && res.data) {
          console.log('labresult', res.data.LabReport);
