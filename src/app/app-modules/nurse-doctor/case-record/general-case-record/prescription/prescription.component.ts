...
export class PrescriptionComponent implements OnInit, OnDestroy {
  ...
  deleteMedicineBackend(index, id) {
    this.doctorService.deleteMedicine(id).subscribe((res) => {
  ...
  counsellingProvidedoneOptionValidation(selectedOption) {
  ...
  patchCounsellingProvided(counsellingProvidedList) {
