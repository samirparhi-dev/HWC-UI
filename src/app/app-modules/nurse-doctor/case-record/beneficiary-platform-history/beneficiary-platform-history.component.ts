...
export class BeneficiaryPlatformHistoryComponent implements OnInit {
  ...
  getMMUHistory() {
    this.doctorService.getMMUHistory().subscribe((data) => {
      ...
      if (data.statusCode == 200) {
        ...
        this.confirmationService.alert(this.current_language_set.unableToLoadData, "error")
      }
    }, err => {
      this.confirmationService.alert(this.current_language_set.unableToLoadData, 'error');
    });
  ...
  getEachVisitData(){
    this.historyOfHWC.forEach((item, i) => {
      if(item.visitCode){
        let reqObj = {
  ...
  filterHWCHistory(searchTerm?: String) {
    if (!searchTerm) {
      ...
      this.historyOfHWC.forEach((item) => {
        const value: string = '' + item.VisitCategory;
        if (value.toLowerCase().indexOf(searchTerm.toLowerCase()) >= 0) {
  ...
  previousHWCHistoryPageChanged(event): void {
    console.log('called', event)
    const startItem = (event.page - 1) * event.itemsPerPage;
    const endItem = event.page * event.itemsPerPage;
    this.previousHWCHistoryPagedList = this.historyOfHWC.slice(startItem, endItem);
