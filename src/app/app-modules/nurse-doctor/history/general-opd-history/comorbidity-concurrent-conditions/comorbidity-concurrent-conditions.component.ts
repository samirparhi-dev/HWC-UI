...
export class ComorbidityConcurrentConditionsComponent implements OnInit, DoCheck {
  ...
  ngOnInit() {
    ...
    this.hrpService.setcomorbidityConcurrentConditions(this.comorbidConditionHrp);
  }
  ngOnChanges()
  {
   //this.comorbidConditionHrp=[];
    ...
    this.confirmationService.confirm(`warn`, this.currentLanguageSet.alerts.info.warn).subscribe(result => {
      if (result) {
        ...
        if (comorbidityConcurrentConditionsList.length == 1 && !!comorbidityConcurrentConditionsForm) {
          ...
          this.comorbidConditionHrp=[];
          for(var a=0;a<comorbidityConcurrentConditionsList.length;a++)
          {
            let comorbidityConditionForm=<FormGroup>comorbidityConcurrentConditionsList.controls[a];
            this.comorbidConditionHrp.push(comorbidityConditionForm.controls["comorbidConditions"].value.comorbidCondition);
          }
          this.hrpService.setcomorbidityConcurrentConditions(this.comorbidConditionHrp);
        }
  ...
  filterComorbidityConcurrentConditionsType(comorbidityConcurrentConditions, i, comorbidityConcurrentConditionsForm?: FormGroup) {
    ...
    if(comorbidityConcurrentConditionsForm !=undefined && comorbidityConcurrentConditions !=undefined && comorbidityConcurrentConditions.comorbidCondition != undefined)
    {
      this.comorbidConditionHrp.push(comorbidityConcurrentConditions.comorbidCondition);
      this.hrpService.setcomorbidityConcurrentConditions(this.comorbidConditionHrp);
    }
    if(comorbidityConcurrentConditionsForm ==undefined && comorbidityConcurrentConditions !=undefined)
    {
