...
export class PastObstericHistoryComponent implements OnInit, DoCheck {
  ...
  getGeneralHistory() {
    this.generalHistorySubscription = this.doctorService.populateHistoryResponse$
      .subscribe(history => {
  ...
  handlePastObstetricHistoryData() {
    ...
    for (let i = 0; i < temp.length; i++) {
      ...
      if (temp[i].pregOrder) {
        ...
        this.resetOtherDeliveryComplication(k, i, false);
        this.resetOtherPregnancyComplication(k, i, false);
  ...
  hrpPastObstetricDetails(setHrpStatus){
    console.log("pastObsteriCHistoryForm", this.pastObstericHistoryForm.controls["pastObstericHistoryList"])
    let pregList=<FormArray>this.pastObstericHistoryForm.controls["pastObstericHistoryList"];
    let ar=[];
    for(var a=0;a < pregList.length;a++)
    {
     let pregGroup=<FormGroup>pregList.controls[a];
     console.log("pregGroup", pregGroup)
     if(pregGroup !== null && pregGroup !== undefined){
     let obj={
       "complicationDuringPregnancy": pregGroup.controls["pregComplicationList"].value,
       "durationOfPregnancy": pregGroup.controls["durationType"].value,
       "typeOfDelivery": pregGroup.controls["deliveryType"].value,
       "deliveryComplication": pregGroup.controls["deliveryComplicationList"].value,
  ...
  resetOtherPregnancyComplication(pastObstericHistoryForm: FormGroup, index, setHrpStatus) {
  ...
  resetOtherDeliveryPlace(pastObstericHistoryForm) {
    ...
    } else if (pastObstericHistoryForm.value.deliveryPlace.deliveryPlace == "Subcentre" || 
  ...
  showOtherDeliveryComplication = false;
  resetOtherDeliveryComplication(pastObstericHistoryForm, index, setHrpStatus) {
