...
export class PastHistoryComponent implements OnInit, DoCheck {
  ...
  femaleSurgeryList = ["Uterine Surgery", "Surgery on Vagina", "Surgery on Cervix", "Dilatation & Curettage (D&C)", "Cesarean Section/LSCS"];
  getMasterData() {
    ...
    .subscribe(masterData => {
        ...
        if (masterData && masterData.illnessTypes && masterData.surgeryTypes) {
          ...
          if (this.beneficiary && (this.beneficiary.genderName.toLowerCase() === "male" || this.beneficiary.ageVal < 12)) {
  ...
  filterPastIllnessTypeInDoctor(illness, i, pastIllnessForm?: FormGroup) {
    ...
    this.confirmationService.confirm(`warn`, this.currentLanguageSet.alerts.info.warn).subscribe(result => {
      if (result) {
        ...
        if (pastIllnessList.length == 1 && !!pastIllnessForm) {
          ...
          this.illnessHRP=[];
          for(var a=0;a<pastIllnessList.length;a++)
          {
            let ilnnessForm=<FormGroup>pastIllnessList.controls[a];
            this.illnessHRP.push(ilnnessForm.controls["illnessType"].value.illnessType);
          }
          this.hrpService.setPastIllness(this.illnessHRP);
  ...
  filterPastIllnessType(illness, i, pastIllnessForm?: FormGroup) {
    ...
    if(illness !=undefined && illness.illnessType != undefined && illness.illnessType.toLowerCase() !== 'other')
    {
      this.illnessHRP.push(illness.illnessType);
      this.hrpService.setPastIllness(this.illnessHRP);
      this.hrpService.checkHrpStatus = true;
  ...
  otherIlnessForHrp(){
    ...
    let otherIllnessHrp = this.pastHistoryForm.controls['pastIllness'].value;
    otherIllnessHrp.forEach(element => { this.illnessHRP.push(element.otherIllnessType)
    });
    this.hrpService.setPastIllness(this.illnessHRP);
    this.hrpService.checkHrpStatus = true;
  }
