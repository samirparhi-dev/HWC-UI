...
export class ObstetricFormulaComponent implements OnInit {
  ...
  setBloodGroupHrp()
  {
    if(this.obstetricFormulaForm.controls["bloodGroup"] !=undefined && this.obstetricFormulaForm.controls["bloodGroup"].value !=undefined)
    this.hrpService.setBloodGroup(this.obstetricFormulaForm.controls["bloodGroup"].value);
  ...
  calculateGravida(formControlName?: string) {
    ...
    if(this.para)
    gravidaValue = gravidaValue + (+this.para);
