...
export class AncDetailsComponent implements OnInit {
  ...
  calculatePeriodOfPregnancy(lastMP) {
    if (lastMP != null) {
      var oneDay = 24 * 60 * 60 * 1000; // hours*minutes*seconds*milliseconds
      var pregnancyMonths = Math.round(Math.abs(Math.round(Math.abs((this.today.getTime() - lastMP.getTime()) / (oneDay)))) / 30);
      if(pregnancyMonths ==0)
      pregnancyMonths=1;
      this.patientANCDetailsForm.patchValue({ duration: pregnancyMonths });
    }
    else {
