...
export class SearchComponent implements OnInit {
  ...
  consentGranted: any;
  searchCategories = ["Beneficiary ID", "Phone No", "Health ID", "HealthID Number", "Family ID", "GovID"];
  ...
  validateSearchTerm(searchTerm, searchObject) {
    if (
  ...
  getSearchResult(reqObj, searchObject){
  ...
  searchBeneficiaryDetails(searchTerm?: string, searchObject?: String){
    if (searchTerm !== undefined && searchTerm !== null && searchTerm.trim().length >= 8 && searchTerm.trim().length <= 32) {
      let reqObj: any;
    if(searchTerm.trim().length == 10  && searchObject.toLowerCase() === "phone no" && searchTerm !== null && searchTerm !== undefined ){
      reqObj = {
    ...
    else if(searchTerm.trim().length == 12  && searchObject.toLowerCase() === "beneficiary id" && searchTerm !== null && searchTerm !== undefined ){
      reqObj = {
        "beneficiaryRegID" : null,
    ...
    else if(searchTerm.trim().length == 17  && searchObject.toLowerCase() === "family id" && searchTerm !== null && searchTerm !== undefined ){
      reqObj = {
        "beneficiaryRegID" : null,
        "beneficiaryID" : null,
        "phoneNo" : null,
    ...
    else if((searchTerm.trim().length == 14 || searchTerm.trim().length == 17)  && (searchObject.toLowerCase() === "health id" || searchObject.toLowerCase() === "healthid number") && searchTerm !== null && searchTerm !== undefined ){
    ...
    else if ((searchTerm.trim().length == 12 || searchTerm.trim().length == 10)  && searchObject.toLowerCase() === "govid" && searchTerm !== null && searchTerm !== undefined ){
      reqObj = {
        "beneficiaryRegID" : null,
        "beneficiaryID" : null,
        "phoneNo" : null,
...
/*JA354063 - QuickSearch on BenID, phone number, ABHA Address, ABHA Number */  
  ...
  validateHealthIDNumberPattern(pattern, healthidval, searchObject) {
    ...
    if (checkPattern) {
      searchObject = healthidval.length === 14 ? 
