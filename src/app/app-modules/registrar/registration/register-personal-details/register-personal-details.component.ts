...
export class RegisterPersonalDetailsComponent implements OnInit {
  ...
  getAlternatePhoneMaps(phoneMap) {
    if (phoneMap && phoneMap.length && phoneMap.length > 0) {
  ...
  changeLiteracyStatus() {
    ...
    literacyStatus.forEach((element, i) => {
      if (element.literacystatusID === this.personalDetailsForm.value.literacyStatus) {
        this.personalDetailsForm.patchValue({
